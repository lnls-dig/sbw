...
beampos_offset = [0.5e-3 -0.25e-3]; % Off-centered beam [m]
%beampos_offset = [0 0];             % Centered beam

chdrift_level = 1e-3;               % Channel gain drift peak-to-peak [ratio]

% Signal amplitude variation [ratio]
bcd_factor = 0;
%bcd_factor = 0.3;                   % Long-range BCD (decay mode)
%bcd_factor = 0.02;                  % Short-range BCD (top-up mode)

% Signal amplitude [V]
%V = 0.02236;                        % -20 dBm @ 50 ohm (0 dBm @ RFFE output)
%V = 0.03159;                        % -17 dBm @ 50 ohm (+3 dBm @ RFFE output)
V = 0.04462;                        % -14 dBm @ 50 ohm (+6 dBm @ RFFE output)
